...
use fe_runtime::shared_node::{SharedNode, PropertyValue, WebViewInteraction, validate_asset_path};
...
fn test_shared_node_transform() {
    ...
    let rot_len = (node.rotation[0].powi(2) 
        ...
        + node.rotation[3].powi(2)).sqrt();
    assert!((rot_len - 1.0).abs() < 0.001, "Quaternion should be normalized");
...
fn test_shared_node_properties() {
    ...
    props.insert("color".to_string(), PropertyValue::String("#FF0000".to_string()));
    ...
    assert_eq!(deserialized.properties.get("color"), Some(&PropertyValue::String("#FF0000".to_string())));
    assert_eq!(deserialized.properties.get("visible"), Some(&PropertyValue::Boolean(true)));
    assert_eq!(deserialized.properties.get("intensity"), Some(&PropertyValue::Number(0.8)));
}
...
fn test_property_value_serialization() {
    ...
    for value in values {
        ...
        let deserialized: PropertyValue = serde_json::from_str(&json).expect("Failed to deserialize");
...
fn test_webview_interaction_variants() {
    ...
    assert!(matches!(interaction, WebViewInteraction::NodeDeselected { node_id } if node_id == "node-123"));
...
fn test_webview_interaction_serialization() {
    let interactions = vec![
        WebViewInteraction::NodeSelected { node: create_test_node() },
        WebViewInteraction::NodeDeselected { node_id: "node-123".to_string() },
    ...
    for interaction in interactions {
        ...
        let deserialized: WebViewInteraction = serde_json::from_str(&json).expect("Failed to deserialize");
