...
fn test_network_round_trip_under_2ms() {
    ...
    evt_rx.recv_timeout(Duration::from_secs(5)).expect("Network thread did not send Started");

    let t0 = Instant::now();
    cmd_tx.send(NetworkCommand::Ping).unwrap();
    let evt = evt_rx.recv_timeout(Duration::from_millis(100)).expect("No Pong");
    let elapsed = t0.elapsed();

    assert!(matches!(evt, NetworkEvent::Pong));
    assert!(elapsed.as_millis() < 2, "Network round-trip {}ms > 2ms", elapsed.as_millis());
}
...
fn test_db_round_trip_under_5ms() {
    ...
    res_rx.recv_timeout(Duration::from_secs(30)).expect("DB thread did not send Started");

    let t0 = Instant::now();
    cmd_tx.send(DbCommand::Ping).unwrap();
    let result = res_rx.recv_timeout(Duration::from_millis(100)).expect("No Pong");
    let elapsed = t0.elapsed();

    assert!(matches!(result, DbResult::Pong));
    assert!(elapsed.as_millis() < 5, "DB round-trip {}ms > 5ms", elapsed.as_millis());
}
...
fn test_clean_shutdown_within_1s() {
    ...
    net_evt_rx.recv_timeout(Duration::from_secs(5)).expect("Network Started");
    db_res_rx.recv_timeout(Duration::from_secs(30)).expect("DB Started");
    ...
    assert!(t0.elapsed() < Duration::from_secs(1), "Threads did not shut down within 1s");
}
