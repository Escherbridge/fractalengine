...
> **Status 2026-07-24.** All open decisions RATIFIED (spec §Ratified decisions):
> Q2 handle-length, Q5 grey-out, Q7 as-designed pick priority, Q8
> full-Illustrator combination anchor; CI-drift stance = track latest stable.
> **Phases 3-7 UNBLOCKED.**
...
Tasks:
- [x] Task: Add `PathOp` variants `AppendSmoothPoint` / `SetAnchorHandles` /
...
- [x] Task: Add matching `UiAction` variants + handlers beside `PathAppendPoint`
      (`actions/mod.rs:593`) and `append_point` (`actions/path.rs:94-109`):
      `append_smooth_point` pushes a full `PathPointRow` + queues the op.
- [x] Task: Extend bridge `advance_path_edits` (`gpx_bridge.rs:1246`) +
      `persist_and_render_points` (`gpx_bridge.rs:1700`) to read-modify-write the
      new slots.
- [x] TDD: bridge round-trip — append smooth, set handles, reload, assert handles
...
Tasks:
- [x] Task: Wire `PointerPhase::phase()` (`router.rs:113`, currently dead code) into
      `handle_path_point_interaction`.
- [x] Task: Add a `PenHandleDrag { anchor_pos, alt_held }` resource (mirror
...
- [x] Task: Release below threshold (~0.15 m) ⇒ existing `append_point` (corner);
      above, no Alt ⇒ `PathAppendSmoothPoint` (symmetric `handle_out=drag`,
      `handle_in=-drag`); above + Alt ⇒ ratified-Q8 combination anchor —
      `handle_out` = final drag, `handle_in` = symmetric value frozen when Alt
      was first pressed (`None` if Alt held from press start), kind `Corner`.
- [x] Task ⚠ (critique must-fix 2): thread `handle_out`/`handle_in`/`corner_code`
      ...
      starts sharp).
- [x] Task: add `ToolPanelState.pen_new_anchor_kind` default (beside `pen_mode`,
      `tool_panel.rs:165`).
- [x] TDD: pure unit tests on the threshold / symmetry / Alt decision (mirroring
...
Tasks:
- [x] Task: Add `HitTarget::PathHandle { idx, side }` +
      ...
      (`MoveHandle` landed position-free — `{ idx, side }` — preserving the
      `Operation` `Eq` derive; consumers compute positions like `MoveVertex`.)
- [x] Task: Spawn handle-marker entities analogous to `PathPointMarker`
...
- [x] Task: On handle drag — Symmetric keeps the opposite handle equal+opposite,
      Smooth keeps direction/frees length, Alt breaks to Corner; commit via
      `SetAnchorHandles`/`SetAnchorCorner`.
- [x] TDD: collinear enforcement + Alt-break transitions; handle-marker claim
...
Tasks:
- [x] Task: In `path_editor_card.rs::render_edit_view` add a "Selected vertex"
...
- [x] Task: Slider auto-derives collinear symmetric handles from the neighbor
      ...
      `SetAnchorCorner`/`SetAnchorHandles`. Q5 RATIFIED: grey the slider out once
      handles are manually asymmetric (`handle_in ≉ −handle_out`), show the
      approximate readback, re-enable via a corner-type toggle re-derive.
- [x] Task ⚠ (critique must-fix 1): add the read-only per-anchor affordance to
...
- [x] TDD: verify all corner-settings mutations target `PathEditorState` + a queued
...
Tasks:
- [x] Task: Extend `fe-ui/src/node_manager/AGENTS.md` §pen-tool (anchor model,
      gesture split, dispatch handle variants) and note the read-only vs editable
      split for the Pen affordance.
- [x] Task: Add a §curve section to `fe-terrain/src/mesh/AGENTS.md`
