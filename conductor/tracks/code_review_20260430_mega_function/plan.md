...
> **Execution note (2026-07-15):** implemented against the current ~30-arm
> state (the arm inventory below predates `QueryResult`,
> `NodeProperties{Loaded,Set,Deleted}`, `FieldDef*`, `PetalTerrainLoaded`,
> `NodeDeleted` — all extracted too). Split into domain submodules
> `verse_manager/db_results/{hierarchy,nodes,roles,tokens,properties,query,fields,terrain}.rs`
> to honor the ~300-line file cap. Per session test policy, `cargo test`/
> `clippy`/`fmt` steps are deferred to the single end-of-session sweep;
> `cargo check -p fe-ui` ran green.
...
- [x] Task 1.1: Audit each match arm's parameter needs
...
- [x] Task 1.2: Write a dispatcher-only smoke test (TDD Red)
...
- [x] Task 2.1: Extract `handle_hierarchy_loaded`
...
- [x] Task 2.4: Extract `handle_entity_renamed` and `handle_entity_deleted`
...
- [x] Task 4.1: Extract `handle_seeded`, `handle_database_reset`, `handle_verse_joined`
  - These send follow-up `DbCommand`s. Extract with `db_sender` param.
  - Run `cargo test -p fe-ui`.

- [x] Task 4.2: Extract logging-only handlers
...
- [x] Task 4.3: Final dispatcher cleanup (clippy/fmt deferred to session-end sweep)
...
- [ ] Task 5.1: Run full test suite (deferred to single end-of-session sweep per test policy)
  - `cargo test -p fe-ui`
  - All existing tests must pass unchanged.

- [x] Task 5.2: Line count check (dispatcher fn ~63 lines; largest submodule ~250 lines)
...
- [x] `apply_db_results` is a thin dispatcher (< 80 lines).
- [x] Each `DbResult` variant handled by a named private function.
- [ ] All existing tests pass. (deferred to end-of-session sweep)
