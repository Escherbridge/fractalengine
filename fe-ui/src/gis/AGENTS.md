...
- `mod.rs` — `GisPanelState` (`open`, `mode`, per-mode input buffers,
  `results: Vec<GisResultRow>`, `query_pending`/`last_error`),
  `GisQueryMode`, `GisResultRow`, and `center_bbox_on` (recomputes the bbox
  min/max text buffers around a given XZ center — used by the panel's
  "Center on Selection"/"Center on Origin" buttons).
- `query.rs` — pure SurrealQL query builders (`annotation_query`,
...
`layer_entries_from_terrain_json`/`set_layer_field`/`LayerUiEntry` (from
`layers.rs`) are re-exported at `crate::gis::*`; `GisPanelState`/
