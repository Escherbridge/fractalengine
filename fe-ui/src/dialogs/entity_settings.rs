//! Entity Settings dialog: General / Access / API Access tabs for Verse,
//! Fractal, and Petal entities.

use bevy_egui::egui;

use super::{ActiveDialog, EntitySettingsType, SettingsTab};
    ...
    egui::Window::new(title)
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui
                    .add(
                        egui::Button::new(egui::RichText::new("General").color(theme::TEXT_BRIGHT))
                            .fill(gen_fill),
                    )
                    .clicked()
                {
                ...
                if ui
                    .add(
                        egui::Button::new(egui::RichText::new("Access").color(theme::TEXT_BRIGHT))
                            .fill(acc_fill),
                    )
                    .clicked()
                {
                ...
                if ui
                    .add(
                        egui::Button::new(egui::RichText::new("API").color(theme::TEXT_BRIGHT))
                            .fill(api_fill),
                    )
                    .clicked()
                {
                    *active_tab = SettingsTab::ApiAccess;
                    ...
                    db_tx
                        .send(DbCommand::ListApiTokens {
                            offset: 0,
                            limit: crate::plugin::API_TOKEN_PAGE_SIZE,
                        })
                        .ok();
                }
            });
            ...
            match current_tab {
                SettingsTab::General => {
                    ...
                    ui.add_space(4.0);
                    ui.horizontal(|ui| {
                        ui.add(egui::TextEdit::singleline(name_buf).desired_width(300.0));
                    ...
                    if *entity_type == EntitySettingsType::Verse {
                        ui.add_space(12.0);
                        ui.label(
                            egui::RichText::new("Default Access")
                        ...
                        if let Some(ref mut da) = default_access_buf {
                            ...
                            if ui
                                .radio_value(&mut is_viewer, true, "Public (viewer) — anyone can see")
                                .clicked()
                            {
                                *da = "viewer".to_string();
                                db_tx
                                    .send(DbCommand::SetVerseDefaultAccess {
                            ...
                            if ui
                                .radio_value(&mut is_viewer, false, "Private (none) — explicit grants only")
                                .clicked()
                            {
                                *da = "none".to_string();
                                db_tx
                    ...
                    if *entity_type == EntitySettingsType::Fractal {
                        ui.add_space(12.0);
                        ui.label(
                            egui::RichText::new("Description").color(theme::TEXT_SECTION).strong(),
                        );
                        ui.add_space(4.0);
                        if let Some(ref mut desc) = description_buf {
                            ...
                            if ui
                                .add(egui::Button::new("Save Description").fill(theme::BG_SAVE))
                                .clicked()
                            {
                                db_tx
                                    .send(DbCommand::UpdateFractalDescription {
                ...
                SettingsTab::Access => {
                    ...
                    if peer_roles.is_empty() && !*roles_loading {
                        ui.label(
                            egui::RichText::new(
                                "No peers known yet. Peers will appear here when they connect.",
                            )
                        ...
                        ui.label(
                            egui::RichText::new("Loading peer roles...")
                                .color(theme::TEXT_MUTED)
                                .italics(),
                        );
                    } else {
                        ...
                        for (i, peer) in peer_roles.iter_mut().enumerate() {
                            ...
                            egui::Frame::NONE.fill(row_bg).inner_margin(egui::Margin::same(4)).show(
                                ui,
                                |ui| {
                                    ui.horizontal(|ui| {
                                        // Online indicator
                                        let dot_color = if peer.is_online {
                                            theme::ICON_ONLINE
                                        } else {
                                            theme::ICON_OFFLINE
                                        };
                    ...
                    ui.add_space(8.0);
                    ui.label(
                        egui::RichText::new("Generate Invite Link").color(theme::TEXT_SECTION).strong(),
                    );
                    ui.add_space(4.0);
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui.add(egui::Button::new("Generate").fill(theme::BG_SAVE)).clicked() {
                            ...
                            let scope =
                                build_entity_scope(entity_type, entity_id, parent_verse_id, parent_fractal_id);
                            db_tx
                                .send(DbCommand::GenerateScopedInvite {
                                    scope,
                ...
                SettingsTab::ApiAccess => {
                    ui.label(
                        egui::RichText::new("API Access Tokens").color(theme::TEXT_SECTION).strong(),
                    );
                    ui.add_space(4.0);
                    ui.label(
                        egui::RichText::new(
                    ...
                    egui::Frame::NONE
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                ui.label(
                                    egui::RichText::new("POST /mcp").monospace().small().color(theme::TEXT_BRIGHT),
                                );
                                ui.add_space(12.0);
                                ui.label(egui::RichText::new("WebSocket:").small().color(theme::TEXT_DIM));
                                ui.label(
                                    egui::RichText::new("GET /ws").monospace().small().color(theme::TEXT_BRIGHT),
                                );
                            });
                        });
                    ...
                    // --- Generate new token ---
                    ui.label(
                        egui::RichText::new("Generate New Token").color(theme::TEXT_SECTION).strong(),
                    );
                    ui.add_space(4.0);
                    ...
                    if let Some(ref token) = generated_api_token {
                        ...
                        egui::Frame::NONE
                            ...
                            .show(ui, |ui| {
                                ui.label(
                                    egui::RichText::new("Token (copy now — shown only once):")
                                        .small()
                                        .color(theme::STATUS_OFFLINE)
                                        .strong(),
                                );
                    ...
                    if *api_tokens_loading {
                        ...
                        ui.label(
                            egui::RichText::new("No active API tokens.").color(theme::TEXT_MUTED).italics(),
                        );
                    } else {
                            ...
                            egui::Frame::NONE.fill(row_bg).inner_margin(egui::Margin::same(4)).show(
                                ...
                                |ui| {
                                    ui.horizontal(|ui| {
                                        ...
                                        ui.label(
                                            egui::RichText::new(&tok.scope).monospace().small().color(theme::TEXT_DIM),
                                        );
                                        ...
                                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                            ...
                                            ui.label(
                                                egui::RichText::new(format!(
                                                    "exp: {}",
                                                    &tok.expires_at[..10.min(tok.expires_at.len())]
                                                ))
                                                .small()
                                                .color(theme::TEXT_MUTED),
                                            );
                    ...
                    ui.label(
                        egui::RichText::new(format!("All API Tokens in this {}", scope_label))
                            .color(theme::TEXT_SECTION)
                            .strong(),
                    );
                    ui.add_space(2.0);
                    ui.label(
                        egui::RichText::new(format!(
                            "Tokens scoped to this {} and all sub-entities.",
                    ...
                    if *scoped_tokens_loading && scoped_api_tokens.is_empty() {
                        ui.label(
                            egui::RichText::new("Loading scoped tokens...").color(theme::TEXT_MUTED).italics(),
                        );
                    } else if scoped_api_tokens.is_empty() {
                        ui.label(
                            egui::RichText::new("No active API tokens in this scope.")
                                .color(theme::TEXT_MUTED)
                                .italics(),
                        );
                    } else {
                        ui.label(
                            egui::RichText::new(format!("{} active token(s)", scoped_api_tokens.len()))
                                .small()
                                .color(theme::TEXT_MUTED),
                        );
                        ui.add_space(4.0);
