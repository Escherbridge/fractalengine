...
use crate::actions::terrain_proposal::MAX_SCULPT_DISTANCE;
...
pub(crate) fn ensure_defaults(tool_panel: &mut ToolPanelState) {
    tool_panel.sanitize_numeric_state();
}
...
pub(crate) fn render_controls_and_emit(
    ...
    ui.horizontal(|ui| {
        ...
        ui.add(
            egui::DragValue::new(&mut tool_panel.terrain_target_height)
                .speed(0.1)
                .range(-MAX_SCULPT_DISTANCE..=MAX_SCULPT_DISTANCE),
        );
    });
    ui.horizontal(|ui| {
        ui.label(egui::RichText::new("Delta").small().color(theme::TEXT_DIM));
        ui.add(
            egui::DragValue::new(&mut tool_panel.terrain_delta)
                .speed(0.1)
                .range(-MAX_SCULPT_DISTANCE..=MAX_SCULPT_DISTANCE),
        );
    });
...
/// area-selection controls (D-A8). Viewport Brush interaction owns commits;
/// this broader panel only edits shared configuration. Kept named
/// `render_sculpt_placeholder` for its frozen call site.
pub(crate) fn render_sculpt_placeholder(ui: &mut egui::Ui, sculpt_state: &mut SculptToolState) {
    sculpt_state.sanitize_numeric_state();
    ui.label(
    ...
    ui.label(
        egui::RichText::new(
            "Only Brush paints in the viewport today; Circle/Rect/Polygon are report-only presets.",
        )
        .small()
        .color(theme::TEXT_MUTED)
        .italics(),
    ...
    match sculpt_state.op {
        SculptOpKind::Level => {
            ui.horizontal(|ui| {
                ...
                ui.add(
                    egui::DragValue::new(&mut sculpt_state.target_height)
                        .speed(0.1)
                        .range(-MAX_SCULPT_DISTANCE..=MAX_SCULPT_DISTANCE),
                );
            });
        ...
        SculptOpKind::Raise | SculptOpKind::Lower => {
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("Delta").small().color(theme::TEXT_DIM));
                ui.add(
                    egui::DragValue::new(&mut sculpt_state.delta)
                        .speed(0.1)
                        .range(-MAX_SCULPT_DISTANCE..=MAX_SCULPT_DISTANCE),
                );
            });
    ...
    ui.label(
        egui::RichText::new(
            "Brush commits from the Brush tool. Defined-shape viewport commits are not implemented yet.",
        )
        .small()
        .color(theme::TEXT_MUTED)
        .italics(),
...
/// Mutable contextual controls shown by the first-class Brush tool.
pub(crate) fn render_brush_controls(ui: &mut egui::Ui, sculpt: &mut SculptToolState) {
    ...
    ui.horizontal(|ui| {
        ui.label(egui::RichText::new("Radius").small().color(theme::TEXT_DIM));
        ui.add(
            egui::DragValue::new(&mut sculpt.radius)
                .speed(0.1)
                .range(0.1..=MAX_SCULPT_DISTANCE)
                .suffix(" m"),
        );
    ...
    ui.horizontal(|ui| {
        ...
        ui.add(egui::Slider::new(&mut sculpt.strength, 0.0..=1.0));
    });
    ui.label(
        egui::RichText::new("Operation")
            .small()
    ...
    ui.horizontal_wrapped(|ui| {
        for op in SculptOpKind::ALL {
            ui.selectable_value(&mut sculpt.op, op, op.label());
        }
    });
    match sculpt.op {
        SculptOpKind::Level => {
            ui.horizontal(|ui| {
                ...
                ui.add(
                    egui::DragValue::new(&mut sculpt.target_height)
                        .speed(0.1)
                        .range(-MAX_SCULPT_DISTANCE..=MAX_SCULPT_DISTANCE),
                );
            });
        ...
        SculptOpKind::Raise | SculptOpKind::Lower => {
            ui.horizontal(|ui| {
                ui.label(egui::RichText::new("Delta").small().color(theme::TEXT_DIM));
                ui.add(
                    egui::DragValue::new(&mut sculpt.delta)
                        .speed(0.1)
                        .range(-MAX_SCULPT_DISTANCE..=MAX_SCULPT_DISTANCE),
                );
            });
        ...
        SculptOpKind::Smooth => {
            ui.label(
                egui::RichText::new("Strength controls the pull toward the region mean.")
                    .small()
                    .color(theme::TEXT_MUTED),
            );
        }
    ...
    ui.horizontal(|ui| {
        ...
        ui.add(egui::TextEdit::singleline(&mut sculpt.material).desired_width(120.0));
    });
}
...
mod tests {
    ...
    #[test]
    fn sculpt_numeric_state_recovers_from_non_finite_values() {
        let mut state = SculptToolState {
            radius: f32::NAN,
        ...
        assert_eq!(state.strength, 0.5);
        assert_eq!(state.target_height, 0.0);
